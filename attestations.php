...
foreach ($periods as $period) {
 ...
 ?></select>
 <label for="attestation_level">Уровень:</level><select id="attestation_level">
 ...
 <label for="attestation_date">Дата:</level><input type="text" class="attestation_date" id="attestation_date" value="<?php echo date("Y-m-d");?>"/>
 </p>
<div>
    <h3>Сдавали:</h3>
...
function attestations_submitted() {
    ...
    $common_fields = "':".join(':',$teachers).":','$period','{$att_levels[$level]}',NULL,'$date',now()";
    $values = [];
    foreach($people as $p) {
        $values[] = "('$p',$common_fields)";
    }
    $query = "INSERT INTO {$wpdb->prefix}attestation (id_man,id_moders,id_period,level,mark,date,dateinsert) VALUES ".implode(',',$values);
